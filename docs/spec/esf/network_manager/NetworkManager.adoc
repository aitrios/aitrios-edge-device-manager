...
|Migrates old-format configuration data to the new format to maintain compatibility with previous versions.
|<<#_FunctionMigration>>
|===
...
Migrates old-format configuration data to the new format to maintain compatibility with previous versions.
* Preconditions +
ParameterStorageManager must be initialized. +
Network Manager must be initialized.
* Function Details
    ** Detailed Behavior +
        *** By calling **EsfNetworkManagerExecMigration** migrates old-format configuration data to the new format. +
        *** By calling **EsfNetworkManagerEraseMigrationSrcData** deletes old-format configuration data. +
    ** Error Behavior and Recovery +
        See <<#_SampleFunctionExecMigration, Data Migration API>> for details in each API description.
...
|Migrates old-format configuration data to the new format.
|<<#_SampleFunctionExecMigration>>

|EsfNetworkManagerEraseMigrationSrcData
|Deletes old-format configuration data.
...
|Normal Mode 
+ Fixed IP
|Set the IP address, subnet mask, gateway, primary DNS, and secondary DNS for the target interface.
|Performed before initiating connection via PL.

|Normal Mode + 
DHCP
|Set the IP address, subnet mask, and gateway of the target interface to 0.0.0.0 (disabled). +
|Performed before initiating connection via PL.

|Normal Mode +
DHCP
|Obtain the IP address using the DHCP client and set the obtained IP address, subnet mask, gateway, primary DNS, and secondary DNS on the target interface. +
...
* *Description* +
** Detailed Behavior +
*** Retrieves current network information for the connection mode of the handle. +
...
*** If called before DHCP processing is complete, the IP address will return a NULL character. +
*** To retrieve the IP address after DHCP completion, the method varies depending on the OS: +
**** NuttX: Call EsfNetworkManagerGetIFInfo after receiving the kEsfNetworkManagerNotifyInfoConnected event via EsfNetworkManagerRegisterCallback. +
**** Linux: Timing is indeterminate. (Event retrieval API not yet implemented) +
...
Return Value +
Returns one of the values of <<#_TableReturnValueResult, EsfNetworkManagerResult>> depending on the execution result.

Description +
** Operation Details +
*** Migrates the following configuration data:
**** IPAddress: IP address +
**** SubnetMask: Subnet mask +
**** Gateway: Gateway +
**** DNS: Primary DNS +
**** IPMethod: Selection of DHCP usage (dhcp / static) +
**** NetifKind: Selection of network interface type (WiFi / Eth) +
**** ProxyURL: Proxy URL +
**** ProxyPort: Proxy port +
**** ProxyUserName: Proxy user name +
**** ProxyPassword: Proxy password
*** Secondary DNS is not included in the migration. +
*** If "IPAddress" cannot be obtained from the old-format configuration data, the new format will not set “IPAddress, SubnetMask, Gateway, Primary DNS”, and “IPMethod” will be set to dhcp. +
*** If "ProxyURL" cannot be obtained from the old-format configuration data, the new format will not set “ProxyURL, ProxyPort, ProxyUserName, ProxyPassword”. +
*** If the old-format configuration data cannot be read or contains invalid values, “IPMethod” will be set to dhcp in the new format.

** Notes +
*** Cannot be called concurrently. +
*** Cannot be called from multiple threads. +
*** Cannot be called from multiple tasks. +
*** This API blocks internally for the following reasons. +
The maximum blocking time is less than 1000 ms. +
**** For exclusive control of resource access +
**** For blocking caused by simultaneous access executions in the Parameter Storage Manager +
...
|kEsfNetworkManagerResultExternalError
|External API error
|Error response from the Parameter Storage Manager API
|Retry; if recovery does not occur after retries, restart the system

|kEsfNetworkManagerResultHWIFError
|PL API error
|Error response from the PL API
|Retry; if recovery does not occur after retries, restart the system

|kEsfNetworkManagerResultInternalError
|Internal error
|Abnormality occurred during internal processing
|Restart the system
...
** Operation Details +
*** Deletes old-format configuration data. +
*** Intended to be called after migrating old-format configuration data to the new format.
...
|normal_mode.dev_ip.ip
|Empty string or IPv4 format parsable by inet_pton
|IPv4 format parsable by inet_pton
|
...
|normal_mode.dev_ip.gateway
|Empty string or IPv4 format parsable by inet_pton
|IPv4 format parsable by inet_pton 
|

|normal_mode.dev_ip.dns
|Empty string or IPv4 format parsable by inet_pton
|Empty string or IPv4 format parsable by inet_pton
|

|normal_mode.dev_ip.dns2
|Empty string or IPv4 format parsable by inet_pton
|Empty string or IPv4 format parsable by inet_pton
|

|normal_mode.dev_ip_v6.*
|Empty string or IPv6 format parsable by inet_pton
|IPv6 format parsable by inet_pton 
|Results in an error when IPv6 is disabled.

|normal_mode.wifi_sta.ssid 
|0–32 chars 
|1–32 chars 
|Content not validated as it's the target SSID

|normal_mode.wifi_sta.password 
|0–64 chars 
|Same as left 
|Content not validated as it's the target password
...
|0–32 ASCII printable chars 
|1–32 ASCII printable chars 
|Characters must be ASCII 33 (!) to 126 (~)

|accesspoint_mode.wifi_ap.password 
|Empty or 8–63 ASCII printable chars or 64 hex chars 
|8–63 ASCII printable chars or 64 hex chars 
|ASCII chars must be in range 33 (!) to 126 (~)
...
* Data Type Definitions
** Added secondary DNS to the following data types
*** EsfNetworkManagerIPInfo
*** EsfNetworkManagerIPInfoMask

* API Definitions
** Added descriptions regarding secondary DNS to the following APIs
*** EsfNetworkManagerStart
*** EsfNetworkManagerExecMigration
