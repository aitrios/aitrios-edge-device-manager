...
*** PLを使用して接続モードに該当するI/Fのイベントハンドラ登録を行います。 +
    IPアドレス、ネットマスク、Gateway、プライマリDNS、セカンダリDNSを設定します。 +
...
|Normalモード +
固定IP
|対象I/FにIPアドレス、ネットマスク、Gateway、プライマリDNS、セカンダリDNSを設定します。
|PLへのネットワーク接続開始前に実行します。

|Normalモード +
DHCP
|対象I/Fに無効(0.0.0.0)IPアドレス、ネットマスク、Gatewayを設定します。 +
|PLへのネットワーク接続開始前に実行します。

|Normalモード +
DHCP
|DHCPCを使用してIPアドレスを取得し、対象I/Fに取得したIPアドレス、ネットマスク、Gateway、プライマリDNS、セカンダリDNSを設定します。 +
...
* *説明* +
** 動作詳細 +
*** ハンドル操作対象の接続モードの現在のネットワーク情報を取得します。 +
...
*** DHCP処理が未完了のタイミングで呼ばれた場合は、IPアドレスはNULL文字を返します。 +
*** DHCP完了後のIPアドレスを取得したい場合は、OSによって異なるため以下の方法で取得します。 +
**** Nuttx：EsfNetworkManagerRegisterCallback で kEsfNetworkManagerNotifyInfoConnected のイベントを受け取ってからEsfNetworkManagerGetIFInfo をコールすること。 +
**** Linux：タイミングは不定。（イベント取得のAPI未実装） +
** 備考
*** 同時に呼び出し可能。
*** 複数のスレッドからの呼び出し可能。
*** 複数のタスクからの呼び出しが可能。
...
*** セカンダリDNSは移行対象外です。 +
*** 旧型式の設定データから「IPAddress」が取得できない場合、新形式では「IPAddress、SubnetMask、Gateway、プライマリDNS」を設定しないで、「IPMethod」をdhcpに設定します。 +
