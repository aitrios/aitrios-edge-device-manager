...
ButtonManagerでは各APIを呼び出すことで<<#_StateTransitionDiagram>>に示す状態遷移を行います。 +
...
* 機能詳細
    ...
    ** 通知用コールバックはAPIによって登録/解除が可能で、コールバックの実行が有効化されている期間に行われたボタン操作が通知対象となる。通知用コールバックが登録されており、ボタン長押し中にコールバックの実行が有効化された場合は、コールバックの実行が有効化されたタイミングから長押しが開始されたとみなす。（<<#_Timing_Diagram_of_Button_Notification_Scope>>を参照）
+
...
+
また、ボタン長押し中にButtonManagerがOpenされた場合のボタン操作通知タイミング図を<<#_Timing_Diagram_When_ButtonManager_Is_Opened_During_Long_Press>>に示す。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerOpen_Return_Values>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerClose_Return_Values>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerRegisterPressedCallback_Return_Values>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerRegisterReleasedCallback_Return_Values>>のいずれかの値が返ります。
+
[#_EsfButtonManagerRegisterReleasedCallback_Return_Values]
.EsfButtonManagerRegisterReleasedCallbackの戻り値の説明
[width="100%", cols="30%,70%",options="header"]
|===
|戻り値  |説明
|kEsfButtonManagerStatusOk
...
既に他のコンテキストでButtonManagerのAPIが動作中の場合は、完了を待ってから実行される。通知用コールバックの実行と被った場合も完了を待ってから実行される。 +
入力引数に不備があった場合は、コールバックを登録せずにエラーを返す。 +
設定可能な値の詳細を<<#_Examples_of_min_second_and_max_second>>に示す。
+
...
|===
+
<<#_Callback_Execution_Judgment_Table_When_Multiple_Release_Callbacks_Registered>>はmin_second、max_secondの条件別のコールバック1～3が同時に登録されている時に、コールバックが実行されるか否かを、ボタン長押し時間毎に表している。〇はコールバック実行され、×はコールバック実行されない。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerRegisterLongPressedCallback_Return_Values>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerUnregisterPressedCallback_Return_Values>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerUnregisterReleasedCallback_Return_Values>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerUnregisterLongPressedCallback_Return_Values>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerEnableNotificationCallback_Return_Values>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfButtonManagerDisableNotificationCallback_Return_Values>>のいずれかの値が返ります。
+
...
<<#_Timing_Diagram_of_Button_Notification_Scope>>を参照。
...
=== 特記事項
* ButtonManagerがCLOSE状態またはDISABLE状態でボタン押下が開始され、ENABLE状態になった後も解放されずに長押しが継続されている場合は、ENABLE状態になった時点から長押し時間の計測を開始する。（<<#_Timing_Diagram_of_Button_Notification_Scope>>を参照）
...
* コンポーネントの機能説明
** ボタン操作通知機能 説明変更/ボタン操作通知範囲のシーケンス図を削除して、ボタン操作通知範囲のタイミング図に変更
** <<#_Timing_Diagram_When_ButtonManager_Is_Opened_During_Long_Press>>を追加
...
** start_secondとend_secondの仕様表をmin_secondとmax_secondの実行例に名称を変更してEsfButtonManagerRegisterReleasedCallbackの説明欄に移動<<#_Callback_Execution_Judgment_Table_When_Multiple_Release_Callbacks_Registered>>をEsfButtonManagerRegisterReleasedCallbackの説明欄に移動
* 汎用的なAPI呼び出しシーケンスの例
...
** optを削除
* 複数モジュールからのAPI呼び出し例<<#_ボタン操作通知範囲のタイミング図>>を参照するように変更・特記事項やコンポーネントごとの特有の説明事項
** 制約等 状態遷移変更に伴い文章変更
** 特記事項 状態遷移変更に伴い文章変更
|v0.0.3
a|
...
|v0.0.4
a|
* <<#_General_API_Call_Sequence_Example>>の軽微な修正を実施
