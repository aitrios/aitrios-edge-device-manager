...
LogManagerでは各APIを呼び出すことで<<#_状態遷移図>>に示す状態遷移を行います。 +
...
* *戻り値* +
+
実行結果に応じて<<#_EsfLogManagerStatus>>のいずれかの値が返ります。
[#_EsfLogManagerStatusの戻り値の説明]
.EsfLogManagerStatusの戻り値の説明
[width="100%", cols="30%,70%",options="header"]
|===
|戻り値  |説明
|kEsfLogManagerStatusOk
...
* *戻り値* +
+
実行結果は常に<<#_EsfLogManagerStatus>>の下記の値が返ります。
[#_EsfLogManagerStatusの戻り値の説明]
.EsfLogManagerStatusの戻り値の説明
[width="100%", cols="30%,70%",options="header"]
|===
...
* *説明* +
Dlog/Elog出力要求時の以下処理を指定します。 設定の詳細は <<#_ログ設定>>を参照して下さい。
** Dlog出力先指定
** Dlogレベル指定
** Elogレベル指定
** Dlogフィルタ指定
...
+
**``[IN] uint32_t size``**:: 
LogManager構成情報より得られるExceptionデータのバッファサイズを指定ください。
上記データサイズ未満の場合、下記バッファにデータは格納されません。

**``[OUT] uint8_t *buf``**:: 
Exception用データ格納バッファ。NULLの場合、上記データサイズ未満の場合、エラーを返します +
エラーの場合、バッファにデータは格納されません。

**``[OUT] uint32_t *out_size``**:: 
実際に格納したExceptionデータのサイズを返します。 +
エラーまたはExceptionデータサイズが0の場合、0を返します。
...
|正常終了
|kEsfLogManagerStatusParamError
|引数のbuf、out_sizeがNULLの場合
|kEsfLogManagerStatusFailed
|異常終了 +
Exceptionデータへのアクセスに失敗、bufがExceptionデータより小さい値の場合
|===

* *説明* +
引数bufに文字列化したExceptionデータを格納します。 +
エラーの場合、引数bufにデータは格納されません。 +
本APIは、多重呼び出し可能です。 
