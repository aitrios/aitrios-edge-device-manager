...
* Function Details
    ...
    ** Supported input formats are listed in <<#_Supported_Input_Formats>>
+
...
[#_MaximumProcessingTime]
==== Maximum Processing Time
The measured values for T3S3 at the time of design are shown in <<#_MaximumProcessingTimeTable>>.
...
[#_HeapMemoryUsage]
==== Heap Memory Usage
The measured values for T3S3 at the time of design are shown in <<#_HeapMemoryUsageTable>>.
...
* *Values*
+
[#_EsfCodecJpegInputFormatValues]
...
* *Return Value* +
+
Returns one of the values defined in <<#_EsfCodecJpegEncodeReturnValues>> depending on the execution result.
+
...
|kJpegParamError
|・`enc_param` is NULL +  
・Invalid value in `enc_param` (see <<#_EsfCodecJpegEncParamValues>> for details) +  
・`jpeg_size` is NULL
...
* *Return Value* +
+
Returns one of the values defined in <<#_EsfCodecJpegEncodeFileIoReturnValues>> depending on the execution result.
+
...
|kJpegParamError
|・`info` is NULL +  
・Invalid value in `info` (see <<#_EsfCodecJpegInfoValues>> for details) +  
・`jpeg_size` is NULL +  
...
* *Return Value* +
+
Returns one of the values defined in <<#_EsfCodecJpegEncodeHandleReturnValues>> depending on the execution result.
+
...
* Moved format descriptions from <<#_Supported_Input_Formats>> to <<#_Terminology>>
