...
=== NTP情報
時刻同期に必要なパラメータである。 +
PrimaryNTPサーバのhost nameもしくはそのIPv4アドレス、SecondaryNTPサーバのhost nameもしくはそのIPv4アドレス（オプション）、およびNTP時刻同期アルゴリズムへの補助入力で構成される。 +
SecondaryNTPサーバは、PrimaryNTPサーバへの接続に失敗した場合や、冗長性を確保するために使用される。 +
NTP情報はParameter Storage Managerを使用して保存される。但し、Parameter Storage Managerで保存する本筋は、動作確認が取れたものに限る。
...
|機能名 |概要  |節番号
|NTP Client等のパラメータ設定
|1. 上位モジュールからPrimaryNTPサーバのhost nameもしくはそのIPv4アドレス、SecondaryNTPサーバのhost nameもしくはそのIPv4アドレス（オプション）、NTP時刻同期に必要なパラメータとClock ManagerのスレッドがNTP Client Daemonを監視する周期を取得する。 +
2. 前項1のパラメータをNTP時刻同期完了を以ってParameter Storage Managerへ保存する。
|<<#_Function1>>

|NTP Client等のパラメータ不揮発からの読み出し
|1. Parameter Storage ManagerからPrimary及びSecondaryNTPサーバのhost nameもしくはそれらのIPv4アドレス、NTP時刻同期に必要なパラメータとClock ManagerのスレッドがNTP Client Daemonを監視する周期を取得する。
|<<#_Function2>>
...
|`connect`
|Primary及びSecondaryNTPサーバのhost name、 +
または、それらのIPv4アドレスを含む接続情報。 +
詳細は<<#_EsfClockManagerConnection>>を参照されたい。
...
==== EsfClockManagerConnection
NTPサーバのhost name、または、IPv4アドレスをもつ構造体である。 +
PrimaryNTPサーバ（hostname）とSecondaryNTPサーバ（hostname2）の2つのNTPサーバを設定できる。
...
|`hostname`
|PrimaryNTPサーバのhost name、 +
または、PrimaryNTPサーバのIPv4アドレス。
|末尾の空文字を含んで<<#_ESF_CLOCK_MANAGER_NTPADDR_MAX_SIZE,ESF_CLOCK_MANAGER_NTPADDR_MAX_SIZE>> +
〔B〕以下。
|`"time.aitrios.sony-semicon.com"`

|`hostname2`
|SecondaryNTPサーバのhost name、 +
または、SecondaryNTPサーバのIPv4アドレス。 +
空文字列の場合、SecondaryNTPサーバは使用されない。
|末尾の空文字を含んで<<#_ESF_CLOCK_MANAGER_NTPADDR_MAX_SIZE,ESF_CLOCK_MANAGER_NTPADDR_MAX_SIZE>> +
〔B〕以下。
|`""` (空文字列)
...
NOTE: PrimaryNTPサーバへの接続に失敗した場合、SecondaryNTPサーバ（hostname2が設定されている場合）が使用される。
...
|`hostname2`
|本構造体のオブジェクトと対になる構造体 `EsfClockManagerConnection` のオブジェクトに属するhostname2が設定済み/欲すのであるならば1、そうでないならば0である。
|1または0
|0
