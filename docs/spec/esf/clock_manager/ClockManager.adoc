...
Consists of the host name or IPv4 address of the primary NTP server, the host name or IPv4 address of the secondary NTP server (optional), and auxiliary inputs for the NTP time-synchronization algorithm. +
The secondary NTP server is used when the primary NTP server connection fails or when redundancy is required. +
...
|1. Obtain the primary NTP server's host name or its IPv4 address, the secondary NTP server's host name or its IPv4 address (optional), the parameters required for NTP time synchronization, and the frequency at which the Clock Manager's threads monitor the NTP Client Daemon from higher-level modules. +
...
|1. Obtain the primary and secondary NTP servers' host names or their IPv4 addresses, the parameters required for NTP time synchronization, and the frequency at which the Clock Manager's threads monitor the NTP Client Daemon from the Parameter Storage Manager.
|<<#_Function2>>
...
|`connect`
|The connection information that includes the host names of the primary and secondary NTP servers, +
or their IPv4 addresses. +
For details, refer to <<#_EsfClockManagerConnection>>. +
|<<#_ESF_CLOCK_MANAGER_NTPADDR_MAX_SIZE,ESF_CLOCK_MANAGER_NTPADDR_MAX_SIZE>> +
bytes or less.
|hostname: "time.aitrios.sony-semicon.com" +
hostname2: "" (empty string) +
...
==== EsfClockManagerConnection
Structure that contains the host name or IPv4 address of the NTP server. +
Two NTP servers can be configured: the primary NTP server (hostname) and the secondary NTP server (hostname2). +
...
|`hostname`
|Host name or +
IPv4 address of the primary NTP server. +
...
|hostname2
|The host name of the secondary NTP server, +
or the IPv4 address of the secondary NTP server. +
If an empty string is specified, the secondary NTP server is not used.
|Including the terminating null character, the size must be less than or equal to <<#_ESF_CLOCK_MANAGER_NTPADDR_MAX_SIZE,ESF_CLOCK_MANAGER_NTPADDR_MAX_SIZE>>.
|"" (empty string)

|===

NOTE: If the connection to the primary NTP server fails, the secondary NTP server (when hostname2 is configured) is used.
...
|hostname2
|Contains 1 if hostname2 in the EsfClockManagerConnection object, which pairs with this structure's object, is set or required; otherwise, contains 0.
|1 or 0
|0
...
[#_thebibliography2]
* [2] ```Parameter Storage Manager Function Specification', Version 0.1.2, https://github.com/aitrios/aitrios-edge-device-manager.git.
<<<
...
|v0.0.8
|Additions +
Added support for secondary NTP.
|===
