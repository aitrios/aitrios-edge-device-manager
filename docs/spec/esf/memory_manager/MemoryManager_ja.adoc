...
（<<#_MemoryInitializationSequence>>を参照）
...
この機能はシステム起動時、メモリマネージャー使用前に実行することを想定しています。 +
※未初期化状態でメモリマネージャーのメモリ操作API実行時はメモリ未初期化のエラーを返します +
（<<#_MemoryFinalizationSequence>>を参照）
...
EsfMemoryManagerAllocateを呼び出す際、”WASMモジュール管理情報"を指定することで該当領域に対するメモリ確保を行います。（<<#_MemoryAllocationSequence>>を参照）
...
この機能は排他的に操作を行うため、複数のスレッドから呼び出しが可能です。
EsfMemoryManagerFreeを呼び出す際、メモリ操作ハンドルを指定することで該当領域に対するメモリ解放を行います。（<<#_MemoryFreeSequence>>を参照）
...
この機能は排他的に操作を行うため、複数のスレッドから呼び出しが可能です。
EsfMemoryManagerMapを呼び出す際、メモリ操作ハンドルを指定することで該当領域に対するメモリマップを行います。（<<#_MemoryMapSequenceDiagram>>を参照）
...
この機能は排他的に操作を行うため、複数のスレッドから呼び出しが可能です。
EsfMemoryManagerUnmapを呼び出す際、メモリ操作ハンドルを指定することで該当領域に対するメモリアンマップを行います。（<<#_MemoryUnmapSequenceDiagram>>を参照）
...
==== Wasmメモリサイズ変更機能
* 機能概要
    ** <<#_WasmMemoryAllocation>>により確保したメモリのサイズ変更を行います。
...
==== Wasmメモリ解放機能
* 機能概要
    ** <<#_WasmMemoryAllocation>>により確保したメモリの解放を行います。
* 前提条件
    ** -
* 機能詳細
...
※メモリ操作ハンドルに紐づくメモリ管理情報は非公開となります(参照：<<#_About_EsfMemoryManagerHandle>>) +
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerAllocateReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFinalizeReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
実行結果に応じて<<#_EsfMemoryManagerAllocateReturnValues>>のいずれかの値を設定します。
+
[#_EsfMemoryManagerAllocateReturnValues]
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFreeReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerMapReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerUnmapReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFopenReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFcloseReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFseekReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFwriteReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFpwriteReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFreadReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerFpreadReturnValues>>のいずれかの値が返ります。
+
...
* *戻り値* +
+
実行結果に応じて<<#_EsfMemoryManagerIsMapSupportReturnValues>>のいずれかの値が返ります。
+
...
EsfMemoryManagerHandleに紐づくメモリ管理情報はユーザーに対して詳細は非公開となります。ここではメモリマネージャー内で管理しているメモリ管理情報の概要を記載します。※あくまでもイメージです
