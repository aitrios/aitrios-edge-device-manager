...
HalErrCode HalDriverInitialize(void) {
  ...
  ret_code = HalDriverInitializeOsImpl();
  if (ret_code != kHalErrCodeOk) {
    LOG_E(0x1E, "HalDriverInitializeOsImpl() failed. ret_code=%u", ret_code);
    goto func_exit;
  }

  struct DriverInfoList *init_drivers_list = NULL;

  ret_code = HalDriverGetInitInfoOsImpl(&init_drivers_list);
  if (ret_code != kHalErrCodeOk) {
    LOG_E(0x1F, "HalDriverGetInitInfoOsImpl() failed. ret_code=%u", ret_code);
    HalDriverFinalizeOsImpl();
    goto func_exit;
  }
...
HalErrCode HalDriverFinalize(void) {
  ...
  ret_code = HalDriverFinalizeOsImpl();
