...
PlErrCode PlButtonRegisterHandler(uint32_t button_id,
  ...
  err_code = PlButtonRegisterHandlerImpl(pin, &btn_status);
  if (err_code != kPlErrCodeOk) {
    LOG_ERR(kPlButtonEventErrorId0x0A,
            "PlButtonRegisterHandlerImpl error=%d.", err_code);
    goto unregister;
  }
...
PlErrCode PlButtonUnregisterHandler(uint32_t button_id) {
  ...
  err_code = PlButtonUnregisterHandlerImpl(pin);
  if (err_code != kPlErrCodeOk) {
    LOG_ERR(kPlButtonEventErrorId0x14,
            "PlButtonUnregisterHandlerImpl error=%d.", err_code);
    goto restore;
  }
...
static PlErrCode ButtonOpen(uint32_t button_id) {
  ...
  err_code = PlButtonOpenImpl(pin);
  if (err_code != kPlErrCodeOk) {
    LOG_ERR(kPlButtonEventErrorId0x29, "PlButtonOpenImpl error=%d.", err_code);
    goto err_free;
  }
...
static PlErrCode ButtonClose(uint32_t button_id) {
  ...
  err_code = PlButtonCloseImpl(pin);
  if (err_code != kPlErrCodeOk) {
    LOG_ERR(kPlButtonEventErrorId0x2C,
            "PlButtonCloseImpl error. err_code=%d, button_id=%d",
            err_code, button_id);
  }
...
static void *ButtonCallbackThread(void *arg) {
  ...
  while (s_thread_enable) {
    PlErrCode err_code = PlButtonWaitEventImpl();
    ...
    err_code = PlButtonGetValImpl(&pin, &button_status);
    if (err_code != kPlErrCodeOk) {
