...
EsfClockManagerNtpTimeSyncStatus EsfClockManagerPostNtpTimeSyncCompleteNotif(
  ...
  if (status == NULL) {
    WRITE_DLOG_ERROR(MODULE_ID_SYSTEM, "%s-%d:%s --- NTP status is NULL.\n",
                     "clock_manager_notification.c", __LINE__, __func__);
    return kEsfClockManagerNtpTimeSyncFailure;
  }

  const PlClockManagerNtpTimeSyncStatus sync_status =
      PlClockManagerJudgeNtpTimeSyncComplete(status);
  switch (sync_status) {
    case kPlClockManagerNtpTimeSyncFailure:
      return kEsfClockManagerNtpTimeSyncFailure;

    case kPlClockManagerNtpTimeSyncSuccess:
      EsfClockManagerPostNotification(kCondTypeIsNtpSyncComplete, true);
      return kEsfClockManagerNtpTimeSyncSuccess;

    case kPlClockManagerHasNotCompletedYet:  // fallthrough
      return kEsfClockManagerHasNotCompletedYet;
